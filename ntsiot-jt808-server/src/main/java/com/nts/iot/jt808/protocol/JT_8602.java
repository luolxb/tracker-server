...
public class JT_8602 implements IMessageBody {
    ...
    public final void readFromBytes(byte[] bytes) {
        ...
        setRectangleAreas(new java.util.ArrayList<>(getAreasCount()));
        int pos = 2;
        while (pos < bytes.length) {
            RectangleAreaItem item = new RectangleAreaItem();
            item.setRectangleAreaId(((bytes[pos] << 24) + (bytes[pos + 1] << 16) + (bytes[pos + 2] << 8) + bytes[pos + 3]));
            item.setRectangleAreaProperty((short) ((bytes[pos + 4] << 8) + bytes[pos + 5]));
            item.setLeftTopLatitude(((bytes[pos + 6] << 24) + (bytes[pos + 8] << 16) + (bytes[pos + 8] << 8) + bytes[pos + 9]));
            item.setLeftTopLongitude(((bytes[pos + 10] << 24) + (bytes[pos + 11] << 16) + (bytes[pos + 12] << 8) + bytes[pos + 13]));
            item.setRightBottomLatitude(((bytes[pos + 14] << 24) + (bytes[pos + 15] << 16) + (bytes[pos + 16] << 8) + bytes[pos + 17]));
            item.setRightBottomLongitude(((bytes[pos + 18] << 24) + (bytes[pos + 19] << 16) + (bytes[pos + 20] << 8) + bytes[pos + 21]));
            pos += 22;
            if ((item.getRectangleAreaProperty() & 0x0001) == 0x0001) {
